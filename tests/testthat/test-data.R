...
test_that("rep data channels", {
  ...
  expect_equal(g2$x$marks[[1]]$data, list(data = 1:5, dates = FALSE, type = "vector"))
  expect_equal(g2$x$marks[[1]]$opts$x, list(data = rep(1.2, 5), dates = FALSE, type = "vector"))
...
test_that("date / time metadata", {
  ...
  g2 <- g |> mark_dot(x = 1:5)
  expect_equal(g2$x$marks[[1]]$data, list(data = 1:5, dates = FALSE, type = "vector"))
  expect_equal(g2$x$marks[[1]]$opts$x, list(data = 1:5, dates = FALSE, type = "vector"))
  g2 <- g |> mark_dot(x = c("foo", "bar"))
  expect_equal(g2$x$marks[[1]]$data, list(data = 1:2, dates = FALSE, type = "vector"))
  expect_equal(g2$x$marks[[1]]$opts$x, list(data = c("foo", "bar"), dates = FALSE, type = "vector"))
  g2 <- g |> mark_dot(x = c("foo", "bar"), y = as.Date(c("2000-01-01", "2100-01-01")), z = 3:4)
  expect_equal(g2$x$marks[[1]]$data, list(data = 1:2, dates = FALSE, type = "vector"))
  expect_equal(g2$x$marks[[1]]$opts$y, list(data = as.Date(c("2000-01-01", "2100-01-01")), dates = TRUE, type = "vector"))
  ...
  expect_equal(g2$x$marks[[1]]$opts$x, list(data = 3, dates = FALSE, type = "scalar"))
  expect_equal(g2$x$marks[[1]]$opts$r, list(data = as.Date(c("2000-01-01")), dates = TRUE, type = "scalar"))
  g2 <- g |> mark_dot(x = 1:10, r = as.Date(c("2000-01-01")))
  expect_equal(g2$x$marks[[1]]$data, list(data = 1:10, dates = FALSE, type = "vector"))
  expect_equal(g2$x$marks[[1]]$opts$r, list(data = rep(as.Date(c("2000-01-01")), 10), dates = TRUE, type = "vector"))
})

test_that("data types", {
  g2 <- g |> mark_dot(x = 1)
  expect_equal(g2$x$marks[[1]]$opts$x$type, "scalar")
  g2 <- g |> mark_dot(x = as.Date("2021-01-05"))
  expect_equal(g2$x$marks[[1]]$opts$x$type, "scalar")
  g2 <- g |> mark_dot(x = as.POSIXct("2021-07-15 15:07:18 CEST"))
  expect_equal(g2$x$marks[[1]]$opts$x$type, "scalar")
  g2 <- g |> mark_dot(x = 1:3)
  expect_equal(g2$x$marks[[1]]$opts$x$type, "vector")
  g2 <- g |> mark_dot(x = c("foo", "bar"))
  expect_equal(g2$x$marks[[1]]$opts$x$type, "vector")
