...
`obsplot` is still an experimental package, in particular its API is not stable and could change in the future, either for self improvements or to follow Observable Plot evolutions. It is not suitable for production right now.
...
We then add a graphical mark to create our chart. Here we use the dot mark by piping the `mark_dot` function. We pass as arguments the `x` and `y` *channels* in the form of character strings giving the corresponding data frame columns :
...
We can also add *constants* options to a mark to modify an attribute in the same way for all dots :
...
To go a bit deeper on the way a chart is build, we have to take a look at the fundamental concepts of Observable Plot : marks, faceting, scales and transforms.
...
- a *column channel*, ie a character string giving the name of a column of `data`
- a *data channel*, ie raw data to be plotted, in general as a vector
- a *constant*, which defines an option (such as size, color...) globally in the same way for all mark elements
...
- If it is a vector of length > 1, it is considered as a data channel
- If it is a single number, it is considered as a data channel except for `r`, `strikeOpacity` and `fillOpacity`
- If it is a single character string, it is considered a column channel except if it is a CSS color for `fill` and `stroke`

You can explicitly specify that a channel is a data channel by using the `as_data()` helper function. In the following example, without `as_data` the code would raise an error as it would look for a `"Paris"` column in the data :
...
When a column or data channel is of type `Date` or `POSIXt` in R, it should automatically be converted to `Date` in JavaScript, and Observable Plot will take it into account for scale specification :
...
[Faceting](https://observablehq.com/@observablehq/plot-facets?collection=@observablehq/plot) allows to create a grid of comparable grouped charts. In Observable Plot faceting is used by adding a `facet` option to `Plot.plot()`. In `obsplot` it is created by piping the `facet` function.
...
[Scales](https://observablehq.com/@observablehq/plot-scales?collection=@observablehq/plot) are a family of functions which allow to modify the way a data value is mapped to a visual attribute such as position, size or color.
...
Every mark allows two provide a set of basic transforms : `filter`, `sort` and `reverse`. Those can be used by passing JavaScript code directly as argument to a mark function :
...
obsplot(penguins) |>
    ...
    mark_tickX(
      transform_groupY(list(x = "median"), y = "species", x = "body_mass_g", stroke = "red")
    ) |>
    scale_x(inset = 6) |>
    scale_y(label = NULL)
```
...
If `width` and `height` are not specified, the chart dimensions in pixels will be determined by Observable Plot. Note that these chart dimensions may not be the same as the HTML widget dimensions, which can produce big margins :
...
Finally, the special value `"auto"` can be used, in this case height and width are computed by `htmlwidgets` and passed to Observable Plot :

```{r}
obsplot(penguins, height = "auto", width = "auto") |>
  mark_dot(x = "bill_length_mm", y = "bill_depth_mm", stroke = "sex")
```

When `obsplot` is used in a Shiny app with a responsive layout such as `fluidPage`, it is recommended to use `"auto"` for width and height so that the chart will redraw itself accordingly when its container is resized.
...
- `data` can be declared in `obsplot()` and inherited by the chart marks, whereas in Observable Plot it can be declared for each mark.
- if a channel is given as a data channel (a vector) and no `data` has been declared, an indexed `data` argument of the same length is automatically added.
- if a channel is considered as data and of size 1, it will be replicated to the length of the greatest other data channel (if possible).
...
But if you want to add new arguments to this predefined list, you'll have to use `append` and put the new arguments themselves in a list :
...
obsplot(penguins, height = 100) |>
  ...
  mark_text(
    transform_group(
      text = "count",
      append(xy, list(fontWeight = "bold", fontSize = 18, stroke = "#FFF", strokeWidth = .5)))
  ) |>
  scale_color(scheme = "PuRd")
```
...
When using JavaScript in `obsplot` with `JS()`, both `d3` and `Plot` libraries are directly available. You can then directly call [d3](https://d3js.org/) functions or [Plot formats](https://github.com/observablehq/plot#formats) in your code.
