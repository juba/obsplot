...
- If it is a single number, it is considered as a data channel except for `r`, `strikeOpacity`,`fillOpacity`, `fontSize` and `rotate`
...
When a column or data channel is of type `Date` or `POSIXt` in R, it is automatically converted to `Date` in JavaScript, and Observable Plot will take it into account for scale specification :
...
By default, a "gear" menu is added on the right side of the plots with additional features such as SVG export. This can be disabled by specifying `menu = FALSE` :

```{r}
obsplot(penguins, menu = FALSE) |>
  mark_dot(x = "bill_length_mm", y = "bill_depth_mm", stroke = "sex")
```
