...
obsplot(alphabet) |>
    ...
    mark_text(x = "letter", y = "frequency", text = JS("d => (d.frequency * 100).toFixed(1)"), dy = -5) |>
    mark_ruleY(0) |>
    scale_x(label = NULL) |>
...
obsplot(travelers) |>
    ...
    mark_text(last_traveler, x = "date", y = "previous", fill = "#8a817c", text = as_data("2019"), dy = "-0.5em") |>
    mark_text(last_traveler, x = "date", y = "current", text = as_data("2020"), dy = "1.2em") |>
    scale_y(grid = TRUE, nice = TRUE, label = "↑ Travelers per day (millions)", transform = JS("d => d / 1e6"))
```
...
obsplot(stateage) |>
    mark_ruleX(0) |>
    mark_tickX(transform_normalizeX(basis = "sum", z = "name", x = "population", y = "age")) |>
    scale_x(axis = "top", label = "Percent (%) →", transform = JS("d => d*100")) |>
